...
display() {
	printf "\e[2m%s\e[22m \e[2m\e[0m|\e[22m\e[0m \e[32m%s\e[0m\n" "$1" "$2"
}

display_active() {
	printf "\e[3m%s\e[22m \e[2m\e[0m|\e[22m\e[0m \e[33m%s\e[0m\n" "$1" "$2"
}
...
all_commits_to_push_pretty() {
	...
	for ((i=${#commits_array[@]} - 1; i >= 0; i--)); do
		...
		printf " \e[2m\e[0m[\e[22m\e[0m$(success $(current_branch))\e[2m\e[0m|\e[22m\e[0m$(notice ${commit_fixed[1]})\e[2m\e[0m]\e[22m\e[0m \e[0m${commit:10}\e[39m\n"
	done
}
...
commit_no_add() {
	...
	if [ $? -eq 0 ]; then
		...
		printf "\e[2m\e[0m[\e[22m\e[0m$(success $branch)\e[2m\e[0m|\e[22m\e[0m$(notice $hash)\e[2m\e[0m]\e[22m\e[0m \e[0m$message_fixed\e[39m\n"
	elif [ $? -eq 1 ]; then
...
amend_no_add() {
	...
	if [ $? -eq 0 ]; then
		...
		printf "\n\e[2msummary:\e[22m\n"
		printf "\t\e[2m\e[0m[\e[22m\e[0m\e[22m\e[0m$(printf "\e[2m\e[33m$BEFORE_HASH\e[0m\e[22m") \e[0m->\e[39m $(notice $AFTER_HASH)\e[2m\e[0m]\e[22m\e[0m \e[2m\e[0m$BEFORE_MESSAGE\e[22m \e[0m->\e[39m \e[0m$AFTER_MESSAGE\e[39m\n"
		...
		if [ $? -eq 0 ]; then
			if [ "$OUTPUT" == "Everything up-to-date" ]; then
				...
				for ((i=${#commits_array[@]} - 1; i >= 0; i--)); do
					...
					printf "\e[2m\e[0m[\e[22m\e[0m$(success $(current_branch))\e[2m\e[0m|\e[22m\e[0m$(notice ${commit_fixed[1]})\e[2m\e[0m]\e[22m\e[0m \e[0m%s\e[39m\n" "${commit:10}"
				done
...
status() {
	printf "\e[2m\e[0m[\e[22m\e[0m$(success "$(current_branch)") \e[0m->\e[39m $(success "$(current_remote)/$(current_branch)")\e[2m\e[0m|\e[22m\e[0m$(notice $(latest_commit_hash))\e[2m\e[0m]\e[22m\e[0m \e[0m%s\e[39m\n" "$(latest_commit_message)"
...
log() {
	git log --pretty=format:"$(printf "\e[2m\e[0m[\e[22m\e[0m")$(success $(current_branch))$(printf "\e[2m\e[0m|\e[22m\e[0m")$(printf "\e[0;33m")%h$(printf "\e[0m")$(printf "\e[2m\e[0m]\e[22m\e[0m \e[0m")%s$(printf "\e[39m")"
}
...
info() {
	...
	if [ $? -eq 0 ]; then
		...
		printf "\e[2m\e[0m[\e[22m\e[0m$(success "$ago")\e[2m\e[0m|\e[22m\e[0m$(notice "$hash")\e[2m\e[0m]\e[22m\e[0m \e[0m$commit_message\e[39m\n"

		body="$(git log -n 1 --date=local --pretty=format:"%b" $hash)"
		if [ ! -z "$body" ]; then
			echo

			printf "\e[0m\t${body//$(printf '\n')/$(printf '\n\t')}\e[39m\n"
		fi
